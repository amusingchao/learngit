...
class THORDiscreteEnvironment(object):
  ...
  def __init__(self, config=dict()):
    ...
    self.h5_file_path = config.get('h5_file_path', '/home/xinchao/workplace/scene_data/%s.h5'%self.scene_name)
  ...
  def reset(self,aa=None):
    ...
    #k = 300
    # reset parameters
    mm=[411,68]    #1000,66 bedroom
    self.current_state_id = mm[aa]
  ...
  @property
  def x_t(self):
    return self.locations[self.terminal_state_id ][0]

  @property
  def z_t(self):
    return self.locations[self.terminal_state_id ][1]

  @property
  def r_t(self):
    return self.rotations[self.terminal_state_id]

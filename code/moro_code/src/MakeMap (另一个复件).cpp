...
Eigen::Matrix3d euler2RotationMatrix(const float roll, const float pitch, const float yaw)  
{  
    Eigen::AngleAxisd rollAngle(roll, Eigen::Vector3d::UnitZ());  
    Eigen::AngleAxisd yawAngle(yaw, Eigen::Vector3d::UnitY());  
    Eigen::AngleAxisd pitchAngle(pitch, Eigen::Vector3d::UnitX());  
...
cv::Mat BuildMap::FloorMap(cv::Mat& inputDepth,float wheel_x,float wheel_y,float wheel_yaw,float head_yaw,float head_pitch,int ID)
{
  ...
  cv::Rect mroi(cv::Point(554,624), cv::Point(153,298));
  cv::Mat xydata(mroi.area(), 3, CV_32F);

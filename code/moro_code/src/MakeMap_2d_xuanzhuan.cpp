...
static float a =-0.0216498292926;//0.045408707;//-0.057114515;//0.0330547082016;//0.084217004;// 0.0330547082016;
static float b =-0.899590669663;//-0.82245082;//-0.83588785;//-0.895213494974;//-0.89223164;// -0.895213494974;
static float c =-0.0497003922012;//0.098771609;//0.06275171;//0.111914819345; //0.028351974;//0.111914819345;
...
//#pragma omp parallel for
  ...
  for(int i=0; i<H*W; i++)
  {
    ...
    camera_to_world_2d(projection_point.at<float>(0,i), projection_point.at<float>(2,i), &tmp_y, &tmp_x, ea_orb.camera_yaw, ea_orb.wheel_xposition, ea_orb.wheel_xposition);
    if(abs(projection_point.at<float>(2,i)+ inorm_abc) <Ground_Height)
    {
      ...
      degree_y = TwoPoints_Degree(ptx.at<float>(i,0), ptx.at<float>(i,2), ea_orb.wheel_xposition, ea_orb.wheel_yposition);   //0 for 
      ...
      if((degree_y<Observe_Degree) && (dist_yuandian<Observe_Distance))
      {
        ...
        int map_flag = fun_2d(tmp_x, tmp_y,&map_index_x,&map_index_y);
        //int map_flag = fun_2d(projection_point.at<float>(0,i), projection_point.at<float>(1,i),&map_index_x,&map_index_y);
        if(Local_Map.at<unsigned char>(map_index_x,map_index_y) == 128)

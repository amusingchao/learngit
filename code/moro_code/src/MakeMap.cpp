...
static float Observe_Middle_Distance = 3; 
...
static float GroundFactor = 30;  //3m for 1+3/60

//static unsigned char Obtacle_Threhold = 20;
//static unsigned char Passible_Threhold = 200;
static unsigned char Local_Obtacle_Val = 5;
static unsigned char Local_Passible_Val = 200;

static unsigned char Global_Obtacle_Val = 0;
static unsigned char Global_Passible_Val = 7;
#define Plane_Test 0
...
BuildMap::BuildMap()
{
  ...
  Global_Map = cv::Mat::zeros(1000, 1000, CV_8UC1);
  
  Global_Map_Count = cv::Mat::ones(1000, 1000, CV_8UC1) * 5;
  
  Img_Save = cv::Mat::zeros(1000,1000,CV_8UC3);
  
  Point_Track = cv::Mat::zeros(1000,1,CV_16S);
  ...
  //Grid_Obtacle_Count = cv::Mat::ones(1000, 1000, CV_8UC1) * 128;
...
cv::Mat BuildMap::FloorMap(cv::Mat& inputRgb, cv::Mat& inputDepth,float wheel_x,float wheel_y,float wheel_yaw,float head_yaw,float head_pitch,int ID)
{
  ...
  Local_Map = cv::Mat::zeros(1000, 1000, CV_8UC1) * 128;
  
  cv::Mat Local_Obtacle_Count;
  //Local_Map = cv::Mat::zeros(1000, 1000, CV_8UC1);
  Local_Obtacle_Count = cv::Mat::ones(1000, 1000, CV_8UC1) * 128;
  
  cv::Mat Local_Ground_Count;
  //Local_Map = cv::Mat::zeros(1000, 1000, CV_8UC1);
  Local_Ground_Count = cv::Mat::ones(1000, 1000, CV_8UC1) * 128;
  
  cv::Mat Local_Map_Draw;
  Local_Map_Draw = cv::Mat::zeros(1000, 1000, CV_8UC1) ;
  //cv::Mat Local_Map;
  //Local_Map = cv::Mat::zeros(1000, 1000, CV_8UC1);
  ...
  int pos_index_x = (-1*wheel_y - Dxlow)/(Dxup - Dxlow)*Dwidth;
  int pos_index_y = 1000-((wheel_x - Dylow)/(Dyup - Dylow)*Dheight);
...
//#pragma omp parallel for
  ...
  rotate_pitch(0,3,ea_orb.camera_pitch, &Yy_tmp_W,&Zz_tmp_W);
  rotate_transfer(Xx_tmp_W, Zz_tmp_W, ea_orb.camera_yaw, ea_orb.wheel_yaw, ea_orb.wheel_xposition, ea_orb.wheel_yposition, &Xx_W,&Zz_W, Input_Num);
  map_flag = fun_2d(Xx_W, Zz_W, &x1_map, &y1_map, Input_Num);
  Xx_tmp_W = 0;
  
  cout<<Point_Track.cols<<endl;
  cout<<Point_Track.rows<<endl;
  //Point_Track.at<int>(Input_Num-1) = x0_map;
  //Point_Track.at<int>(Input_Num) = y0_map;
  Point_Track_2[Input_Num][0] = x0_map;
  Point_Track_2[Input_Num][1] = y0_map;
  //cout<<x0_map <<"   "<<y0_map<<endl;
  //cout<<Point_Track.at<int>(Input_Num,0) <<"   "<<Point_Track.at<int>(Input_Num,1)<<endl;
  //bool tmp_flag;
  for(int i=0; i<H*W; i++)
  {
    //cout<<"1222222"<<endl;
    ...
    //dist_yuandian = TwoPoints_Diostance(X_W, Z_W, Xx_W,Zz_W);
    dist_yuandian = TwoPoints_Diostance(X_W, Z_W, -1*wheel_y, wheel_x);
    //cout<<"as"<<endl;
    if(dist_yuandian<Observe_Middle_Distance)
    {
      if(abs(dist_1) < (Ground_Height + dist_yuandian / GroundFactor)) 
        {
          //tmp_flag 
          ...
          if(degree_y < Observe_Degree)
            {
              ...
              if((Local_Ground_Count.at<unsigned char>(map_index_y,map_index_x) + Value2) <= 255)
               {
                
                Local_Ground_Count.at<unsigned char>(map_index_y,map_index_x) += Value2;
               }
               /*if((Img_Save.at<Vec3b>(map_index_y,map_index_x)[0] + Value2) <= 255)
               {
                Img_Save.at<Vec3b>(map_index_y,map_index_x)[0] += Value2;//B    
                Img_Save.at<Vec3b>(map_index_y,map_index_x)[1] += Value2;//G    
                Img_Save.at<Vec3b>(map_index_y,map_index_x)[2] += Value2;
                //Local_Map.at<unsigned char>(map_index_y,map_index_x) += Value2;
               }*/
            }
        } 
       if(dist_1 > (Ground_Height + dist_yuandian / GroundFactor) && dist_1 < Tianhuaban_Height)  //(dist_1>0.2 && dist_1<Tianhuaban_Height)   //0.1
        {
          //tmp_flag 
          ...
          if(degree_y < Observe_Degree)
            {
              map_flag = fun_2d(X_W, Z_W,&map_index_x,&map_index_y,Input_Num);  //y for hang x for lie
              //Global_Map = update_global(map_index_y, map_index_x, Ground_Flag);
              if((Local_Obtacle_Count.at<unsigned char>(map_index_y,map_index_x)-Value1) > 0)
              {
                Local_Obtacle_Count.at<unsigned char>(map_index_y,map_index_x) = Local_Obtacle_Count.at<unsigned char>(map_index_y,map_index_x) - Value1;
                //inputRgb.at<unsigned char>(jj,ii) = 0;
                //Grid_Obtacle_Count
  ...
  //for global_modify
  unsigned char ground_val = 0;
  unsigned char obtacle_val = 0;

  //for local_map mix and global_map mix
  for( int nrow = 0; nrow < Local_Obtacle_Count.rows; nrow++)  
  {  
     for(int ncol = 0; ncol < Local_Obtacle_Count.cols; ncol++)  
     {  
         ground_val = Local_Ground_Count.at<unsigned char>(nrow,ncol);
         obtacle_val = Local_Obtacle_Count.at<unsigned char>(nrow,ncol);
         if(obtacle_val < Local_Obtacle_Val)
         {
           Local_Map.at<unsigned char>(nrow,ncol) = 128;
           if(Global_Map_Count.at<unsigned char>(nrow,ncol) > 0)
            Global_Map_Count.at<unsigned char>(nrow,ncol) = Global_Map_Count.at<unsigned char>(nrow,ncol) - 1;
         }
         else
         {
           if(ground_val > Local_Passible_Val)
             {
               Local_Map.at<unsigned char>(nrow,ncol) = 255;
               if(Global_Map_Count.at<unsigned char>(nrow,ncol) < 10)
                 Global_Map_Count.at<unsigned char>(nrow,ncol) = Global_Map_Count.at<unsigned char>(nrow,ncol) + 1;
             }
         }
         //cout<<val<<endl;     
  ...
  unsigned char global_ground_val = 0;
  unsigned char global_obtacle_val = 0;
  for( int g_nrow = 0; g_nrow < Global_Map_Count.rows; g_nrow++)  
  {  
     for(int g_ncol = 0; g_ncol < Global_Map_Count.cols; g_ncol++)  
     {  
         global_ground_val = Global_Map_Count.at<unsigned char>(g_nrow,g_ncol);
         
         if(global_ground_val == 5)
         {
           
            Global_Map.at<unsigned char>(g_nrow,g_ncol) = 0;
         }
         if(global_ground_val > 5)
         {
           
            Global_Map.at<unsigned char>(g_nrow,g_ncol) = 255;
         }
         if(global_ground_val < 5)
         {
           
            Global_Map.at<unsigned char>(g_nrow,g_ncol) = 128;
         }               
     }  
   }
  ...
  //line(Global_Map,Point(x0_map,y0_map),Point(x1_map,y1_map),Scalar(128,128,128),1,CV_AA);  
  //line(Img_Save,Point(x0_map,y0_map),Point(x1_map,y1_map),Scalar(255,0,0),1,CV_AA); 
  //cv::circle(image, pointInterest, 2, cv::Scalar(0, 0, 255));//在图像中画出特征点，2是圆的半径 
  //cv::circle(Local_Map_Draw, Point(x0_map,y0_map), 75, cv::Scalar(0, 0, 0));
  
  //cv::circle(Img_Save, Point(x0_map,y0_map), 2, cv::Scalar(255, 0, 0));
  
  //draw track point
 //cout<<Input_Num<<endl;
 //cout<<Point_Track_2<<endl;
  ...
  sprintf(ee,"%d",Input_Num+3900);  
  //cout<<"save:"<<cc<<endl;
  cv::imwrite(strcat(dd,cmd11), inputRgb);
  cv::imwrite(strcat(ee,cmd11), Local_Map);
  pose_save[Input_Num-1][0] = ea_orb.wheel_xposition;
  pose_save[Input_Num-1][1] = ea_orb.wheel_yposition;
  pose_save[Input_Num-1][2] = ea_orb.wheel_yaw;
  pose_save[Input_Num-1][3] = ea_orb.camera_pitch;
  pose_save[Input_Num-1][4] = ea_orb.camera_yaw;
  
  //
  //fclose(f1);
  FILE *f1 = fopen("f4.txt","w");
  if(Input_Num == 50)
    cout<<pose_save[0][0]<<endl;
    for(int f_i=0; f_i<Input_Num; f_i++)
      {
      fprintf(f1,"%f %f %f %f %f\n",pose_save[f_i][0],pose_save[f_i][1],pose_save[f_i][2],pose_save[f_i][3],pose_save[f_i][4]);
      cout<<pose_save[f_i][0]<<endl;
      }
  fclose(f1);
  //cv::imwrite(strcat(dd,cmd11), Img_Save);
  return Global_Map;
  //return Local_Map;
}

...
void rotate_transfer(float xx_p,float zz_p,float total_yaw,float transfer_x,float transfer_y,float *xx_w,float *zz_w,int mmm)
{    
  ...
  *zz_w = zz_p * cos(total_yaw) + xx_p * sin(total_yaw)+ transfer_x + Camera_To_Robot;
  if(0)//(mmm==17)
  {
    cout<<"indef:"<<endl;
    cout<<xx_p<<endl;
...
cv::Mat BuildMap::FloorMap(cv::Mat& inputRgb, cv::Mat& inputDepth,float wheel_x,float wheel_y,float wheel_yaw,float head_yaw,float head_pitch,int ID)
{
  ...
  ea_orb.camera_yaw = (wheel_yaw + head_yaw) ;//wheel_yaw + head_yaw ;
...
//#pragma omp parallel for
  ...
  for(int i=0; i<H*W; i++)
  {
    ...
    {
      if(abs(dist_1)<Ground_Height)//(abs(projection_point.at<float>(2,i)+ inorm_abc) <Ground_Height)
      {       
        ...
        rotate_transfer(world_ptx.at<float>(0,i),Z_tmp_W,ea_orb.camera_yaw,ea_orb.wheel_xposition,ea_orb.wheel_yposition,&X_W,&Z_W,Input_Num);
        ...
        if((degree_y<Observe_Degree) && (dist_yuandian<Observe_Distance))
          {
            //Z_W = -1*Z_W;
            //int map_flag = fun_2d(X_tmp_W, Z_tmp_W,&map_index_x,&map_index_y,Input_Num);
            /*if(Input_Num ==100)
            {
              cout<<"sin:"<<sin(ea_orb.camera_yaw)<<endl;
              cout<<"cos:"<<cos(ea_orb.camera_yaw)<<endl;
              cout<<world_ptx.at<float>(2,i)<<endl;
              cout<<Z_tmp_W<<endl;
            }*/
            int map_flag = fun_2d(X_W, Z_W,&map_index_x,&map_index_y,Input_Num);
            /*if(Input_Num ==100)
            {
            cout<<Z_W<<endl;
            cout<<map_index_y<<endl;
            }*/
            Local_Map.at<unsigned char>(map_index_y,map_index_x) = 255;
            //Last_Map.at<unsigned char>(map_index_y,map_index_x) = 255;
            Local_no_obtacle_Map.at<unsigned char>(map_index_y,map_index_x) = 255;
            //inputRgb.at<unsigned char>(jj,ii) = 0;
          }   
      }  
      if(0)//(dist_1<0)//(abs(projection_point.at<float>(2,i)+ inorm_abc) <Ground_Height)
      {       
        rotate_pitch(world_ptx.at<float>(1,i),world_ptx.at<float>(2,i),ea_orb.camera_pitch, &Y_tmp_W,&Z_tmp_W);
        ...
        if(1)//((degree_y<Observe_Degree) && (dist_yuandian<Observe_Distance))
          {
            //Z_W = -1*Z_W;
            //int map_flag = fun_2d(X_tmp_W, Z_tmp_W,&map_index_x,&map_index_y,Input_Num);
            /*if(Input_Num ==100)
      ...
      if(dist_1>Ground_Height && dist_1<Tianhuaban_Height)//(abs(projection_point.at<float>(2,i)+ inorm_abc) <Ground_Height)
      {
        ...
        if(1)//((degree_y<Observe_Degree) && (dist_yuandian<Observe_Distance))
        {
          ...
          Local_Map.at<unsigned char>(map_index_y,map_index_x) = 128;
    ...
    if(dist_yuandian>Observe_Middle_Distance && dist_yuandian < Observe_Distance)
    {
      
      if(abs(dist_1)<Ground_Farway_Height)//(abs(projection_point.at<float>(2,i)+ inorm_abc) <Ground_Height)
      {       
        rotate_pitch(world_ptx.at<float>(1,i),world_ptx.at<float>(2,i),ea_orb.camera_pitch, &Y_tmp_W,&Z_tmp_W);
      ...
      if(dist_1>Ground_Farway_Height && dist_1<Tianhuaban_Height)//(abs(projection_point.at<float>(2,i)+ inorm_abc) <Ground_Height)
      {
  ...
  cv::Mat Local_for_dilate(1000, 1000, CV_8UC1);
  Local_for_dilate.setTo(0);
  dilate(Local_no_obtacle_Map, Local_for_dilate, element); 
  image_diff = Local_for_dilate-Local_Map;
  
  cv::Mat outputMap_draw(1000, 1000, CV_8UC1);
  outputMap_draw.setTo(0);
  for( int nrow = 0; nrow < image_diff.rows; nrow++)  
  {  
     for(int ncol = 0; ncol < image_diff.cols; ncol++)  
     {  
         unsigned char val = image_diff.at<unsigned char>(nrow,ncol);
         ...
         if((val<130)&&(val>120))
         {
          ...
          outputMap_draw.at<unsigned char>(nrow,ncol)=128;
         } 
         if((val==0)&&(Local_no_obtacle_Map.at<unsigned char>(nrow,ncol)==255))
         {
          //cout<< val<<endl;
          Last_Map.at<unsigned char>(nrow,ncol)=255;
          outputMap_draw.at<unsigned char>(nrow,ncol)=255;
         } 
         //cout<<val<<endl;     
     }  
  } 
  ...
  char dd[10];  
  sprintf(dd,"%d",Input_Num+2400);  
  //cout<<"save:"<<cc<<endl;
  cv::imwrite(strcat(dd,cmd11), inputRgb);
 
  //return Local_Map;
  //return outputMap_draw;   
  return Last_Map;
}

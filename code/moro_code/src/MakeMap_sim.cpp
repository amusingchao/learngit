...
static float a =-0.0260110122952;//0.045408707;//-0.057114515;//0.0330547082016;//0.084217004;// 0.0330547082016;
static float b =-0.651447857588;//-0.82245082;//-0.83588785;//-0.895213494974;//-0.89223164;// -0.895213494974;
static float c =-0.583221069951;//0.098771609;//0.06275171;//0.111914819345; //0.028351974;//0.111914819345;
...
{    
  ...
  *zz_w = zz_p * cos(total_yaw) + xx_p * sin(total_yaw)+ transfer_x;
  if(0)//(mmm==17)
  {
    cout<<"indef:"<<endl;
    cout<<xx_p<<endl;
...
//#pragma omp parallel for
  ...
  for(int i=0; i<H*W; i++)
  {
    ...
    if(abs(dist_1)<Ground_Height)//(abs(projection_point.at<float>(2,i)+ inorm_abc) <Ground_Height)
    {
      ...
      //Z_tmp_W = world_ptx.at<float>(1,i);
      //Y_tmp_W = world_ptx.at<float>(2,i);
  ...
  char dd[8];  
  sprintf(dd,"%d",Input_Num+601);  
  //cout<<"save:"<<cc<<endl;
  cv::imwrite(strcat(dd,cmd), inputRgb);

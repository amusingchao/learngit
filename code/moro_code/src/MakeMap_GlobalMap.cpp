...
//#pragma omp parallel for
  ...
  for(int i=0; i<H*W; i++)
  {
    ...
    if(dist_yuandian<Observe_Middle_Distance)
    {
      if(abs(dist_1)<Ground_Height) 
       ...
       if(dist_1>0.2 && dist_1<Tianhuaban_Height)   //0.1
        {
          //tmp_flag 
          rotate_pitch(world_ptx.at<float>(1,i),world_ptx.at<float>(2,i),ea_orb.camera_pitch, &Y_tmp_W,&Z_tmp_W);
          X_tmp_W = world_ptx.at<float>(0,i);
  ...
  Xx_tmp_W = 0;
  rotate_transfer(Xx_tmp_W,Zz_tmp_W,ea_orb.camera_yaw,ea_orb.wheel_xposition,ea_orb.wheel_yposition,&Xx_W,&Zz_W,Input_Num);
  map_flag = fun_2d(Xx_W, Zz_W, &x0_map, &y0_map, Input_Num);
  
  rotate_pitch(0,1,ea_orb.camera_pitch, &Yy_tmp_W,&Zz_tmp_W);
  Xx_tmp_W = 0;
  rotate_transfer(Xx_tmp_W,Zz_tmp_W,ea_orb.camera_yaw,ea_orb.wheel_xposition,ea_orb.wheel_yposition,&Xx_W,&Zz_W,Input_Num);
  map_flag = fun_2d(Xx_W, Zz_W, &x1_map, &y1_map, Input_Num);
  
  line(Last_Map,Point(x0_map,y0_map),Point(x1_map,y1_map),Scalar(120,120,120),1,CV_AA);  
